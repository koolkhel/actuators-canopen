...
void SDOTimeoutAlarm(CO_Data* d, UNS32 id)
{
	...
	MSG_ERR(0x1A01, "SDO timeout. SDO response not received.", nodeId);
...
UNS8 lineToSDO (CO_Data* d, UNS8 line, UNS32 nbBytes, UNS8* data) {
	UNS8 i;
	UNS32 offset;
	//	printf("lineToSDO sendind %d bytes, offset %d, with count as %d\n", nbBytes, d->transfers[line].offset + nbBytes, d->transfers[line].count);
...
UNS8 proceedSDO (CO_Data* d, Message *m)
{
	...
	MSG_WAR(0x3A60, "proceedSDO cob", UNS16_LE(m->cob_id));
	whoami = SDO_UNKNOWN;
	...
	/* indigo: our special case is other, middle block download is cs 7, so no need for cs 6 */
   	if (!err) {
	...
	//	printf("indigo: CS == 0x%02hhX, err == 0x%02hhX\n", cs, err);
	...
	case 5: /* Command specifier for data transmission - the client or server is the data producer */
		...
		if (whoami == SDO_SERVER) { /* Server block upload */
			if (SubCommand == SDO_BCS_INITIATE_UPLOAD_REQUEST) {
				...
				int MaxSeqNo = nbBytes / 7 + 1;
				int send_bytes = nbBytes;
				int SeqNo = 1;
				int i = 0;

				/* indigo: 1 byte for frame count, 3 bytes for overall data length */
				data[4] = (UNS8) MaxSeqNo; /* 7 bytes per frame */
				data[5] = (UNS8) nbBytes;
				data[6] = (UNS8) (nbBytes >> 8);
				data[7] = (UNS8) (nbBytes >> 16);
				...
				for (SeqNo = 1; SeqNo <= MaxSeqNo; SeqNo++) {
					//while (true) {

					d->transfers[line].lastblockoffset = (UNS8) d->transfers[line].offset;

					send_bytes = 0;
					getSDOlineRestBytes(d, line, &send_bytes);
					assert(send_bytes > 0);
					MSG("SDO UPLOAD send_bytes = %d, SeqNo = %d, MaxSeqNo\n", send_bytes, SeqNo, MaxSeqNo);
					if (send_bytes > 7) {
						/* middle frame */
						data[0] = (7 << 5) | SeqNo;
						err = lineToSDO(d, line, 7, data + 1);
						...
						sendSDO(d, whoami, CliServNbr, data);
						MSG_WAR(0x9000, "SDO. Sending upload segment defined at index 0x1200 SeqNo ", SeqNo);


						d->transfers[line].offset = d->transfers[line].lastblockoffset + 7;
					} else {
						/* last frame */
						data[0] = (6 << 5) | ((7 - send_bytes) << 2) | 1;
						err = lineToSDO(d, line, send_bytes, data + 1);
						if (err) {
							failedSDO(d, CliServNbr, whoami, index, subIndex, SDOABT_GENERAL_ERROR);
							return 0xFF;
						}

						for (i = send_bytes + 1 ; i < 8 ; i++)
						        data[i] = 0;

						sendSDO(d, whoami, CliServNbr, data);
						MSG_WAR(0x9001, "SDO. Sending LAST upload segment defined at index 0x1200 SeqNo ", SeqNo);

						d->transfers[line].endfield = (UNS8) (7 - send_bytes);
						d->transfers[line].offset = d->transfers[line].lastblockoffset + send_bytes;

						assert(SeqNo == MaxSeqNo); // last loop cycle
					}
				...
				MSG_ERR(0x9002, "SDO error : situation we prevously considered as impossible occured as planned, nodeId", nodeId);
				/* A SDO transfer should have been yet initiated. */
				if (!err)
					err = d->transfers[line].state != SDO_BLOCK_UPLOAD_IN_PROGRESS;
				if (err) {
	...
	case 7: /* middle blocks for block upload come as CS=7 */
		if (whoami == SDO_SERVER) { /* Server block download */
			if (err) { /* indigo: err == no SDOtoLine found */
				...
				data[1] = (UNS8) d->transfers[line].index;        /* LSB */
				data[2] = (UNS8) (d->transfers[line].index >> 8); /* MSB */
				data[3] = d->transfers[line].subIndex;
				...
				/* indigo: this message is unnecessary in chineese case */
				sendSDO(d, whoami, CliServNbr, data);
			}
			    ...
			    if (d->transfers[line].rxstep == RXSTEP_INIT) {
				    ...
				    if ((m->data[0] & 1) == SDO_BSS_INITIATE_UPLOAD_RESPONSE) {
					    ...
					    d->transfers[line].peerCRCsupport = ((m->data[0])>>2) & 1;
					    if ((m->data[0]) & 2)	/* if data set size is indicated */
						    d->transfers[line].objsize = (UNS32)m->data[5] + (UNS32)m->data[6]*256 + (UNS32)m->data[7]*256*256;
					    d->transfers[line].count = (UNS32)m->data[5] + (UNS32)m->data[6]*256 + (UNS32)m->data[7]*256*256;
					    //					    printf("count is %d\n", d->transfers[line].count);
					    // data[4] contains frame_count
					    d->transfers[line].seqno = data[4];
					    data[0] = (5 << 5) | SDO_BCS_START_UPLOAD;
					    for (i = 1 ; i < 8 ; i++)
						    data[i] = 0;
					    MSG_WAR(0x3AB6, "SDO. Sending block upload start to node id ", nodeId);
					    /* indigo: phase II is not by chineese */
					    //sendSDO(d, whoami, CliServNbr, data);
				    }
			    }
			    else if (d->transfers[line].rxstep == RXSTEP_STARTED) {
				    //				    printf("RXSTEP_STARTED 0x%02hhX\n", m->data[0]);
				    /* indigo: seq number is 5 bits && command is 7 */
				    SeqNo = m->data[0] & 0x1F;
				    if (((m->data[0] & (6 << 5)) == (m->data[0] & 0xE0))
					    && (m->data[0] & 0x1)) {	/* Last segment ? */
					    ...
					    /* indigo: confirmations should not be sent */
					    d->transfers[line].seqno = 0;

					    /* copied code from RXSTEP_END */
					    NbBytesNoData = (m->data[0]>>2) & 0x07;
					    /* Store the data in the transfer structure. */
					    err = SDOtoLine(d, line, 7-NbBytesNoData, d->transfers[line].tmpData + 1);
					    if (err) {
					    ...
					    if(d->transfers[line].objsize){ /* If size was indicated in the initiate request */
						    if (d->transfers[line].objsize != d->transfers[line].offset){
							    ...
							    printf("sdo.c,1822: indigo, received %u against %u\n", d->transfers[line].objsize, d->transfers[line].offset);
							    //							    failedSDO(d, CliServNbr, whoami, d->transfers[line].index, d->transfers[line].subIndex, SDOABT_LOCAL_CTRL_ERROR);
							    //return 0xFF;
						    }
					    }
					    ...
					    /* indigo: should put object index and subindex instead of zeroes */
					    for (i = 4 ; i < 8 ; i++)
						    data[i] = 0;

					    MSG_WAR(0x9004, "SDO. Sending block upload end request to node id ", nodeId);
					    sendSDO(d, whoami, CliServNbr, data);
					    MSG_WAR(0x9005, "SDO. End of block upload request", 0);
					    StopSDO_TIMER(line);
					    d->transfers[line].state = SDO_FINISHED;
					    if(d->transfers[line].Callback) (*d->transfers[line].Callback)(d,nodeId);
				    }
					    ...
					    //					    printf("WAY 2\n");
					    // uncomment this when you realise sequence is not guaranteed
					    //if (SeqNo == (d->transfers[line].seqno + 1)) {
					    RestartSDO_TIMER(line);

					    // todo set correct offset based on seqno
					    d->transfers[line].seqno = SeqNo;
					    /* Store the data in the transfer structure. */
					    err = SDOtoLine(d, line, 7, (*m).data + 1);
					    if (err) {
				    ...
				    printf("RXSTEP_END -- should NOT BE HERE!!!\n");
				    /* here store remaining
 bytes in tmpData to line, check size and confirm or abort */
