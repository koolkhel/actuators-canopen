      ...
      if (Size && ((offset + Size) <= 64))
        {
          ...
          if ((result = getODentry (d, index, subIndex, tmp, &ByteSize, &dataType, 0)) !=
              OD_SUCCESSFUL)
            {
              MSG_ERR (0x9005,
                       " getODentry returned : ",
...
sendOnePDOevent (CO_Data * d, UNS8 pdoNum)
{
  ...
  if (!d->CurrentCommunicationState.csPDO ||
    ...
    {
	    printf("INHIBITED PDO!!! %hhd %hhd\n", d->CurrentCommunicationState.csPDO, d->PDO_status[pdoNum].transmit_type_parameter);
      return 0;
    }
