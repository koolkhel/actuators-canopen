...
#define INDIGO_LOG(fmt, ...) fprintf(stdout, fmt, # __VA_ARGS__);
//#define INDIGO_LOG(fmt, ...)

pthread_mutex_t kvaser_mutex = PTHREAD_MUTEX_INITIALIZER;
/**
...
unsigned long timeStamp;
    ...
    pthread_mutex_lock(&kvaser_mutex);
    /* checking for input message (blocking) */
    retval = canReadWait((int)fd0, (long*)&m->cob_id, &m->data, (unsigned*)&m->len, &flags, &timeStamp, 1);
    if ((retval != canOK) && (retval != canERR_NOMSG))
    {
	    //INDIGO_LOG("canReceive_driver (Kvaser) : canReadWait() error, cob_id=%08uX, len=%hhu, flags=%08X, returned value = %d\n",
	    //m->cob_id, m->len, flags, retval);
    	pthread_mutex_unlock(&kvaser_mutex);
        //canClose((int)fd0);
        return retval;
    }
    pthread_mutex_unlock(&kvaser_mutex);

    if (retval != canERR_NOMSG) {
	    m->rtr = 0;
	    if (flags & canMSG_RTR)
	    {
		    m->rtr = 1;
	    ...
	    //INDIGO_LOG("canReceive_driver (Kvaser) : canReadWait() received packet, cob_id=%08uX, len=%hhu, flags=%08X, timestamp=%lu  returned value = %d\n",
	    //		    m->cob_id, m->len, flags, timeStamp, retval);
	    // INDIGO_LOG("RECEIVE: ");
	    printf("RECEIVE: %ld.%ld ", tv.tv_sec, tv.tv_usec);
	    print_message(m);
	    printf("\n");
    } else {
	    retval = 120;
...
unsigned flags = 0;
    ...
    pthread_mutex_lock(&kvaser_mutex);
    retval = canWrite((int)fd0, m->cob_id, m->data, m->len, flags);
    if (retval != canOK)
    {
  	    //INDIGO_LOG("canSend_driver (Kvaser) :  canWriteWait() error, cob_id=%08X, len=%u, flags=%08X, returned value = %d\n",
	    //  m->cob_id, m->len, flags, retval);
	    //canClose((int)fd0);
	    printf("SEND: %ld.%ld error sending message: %d\n", tv.tv_sec, tv.tv_usec, retval);
    	pthread_mutex_unlock(&kvaser_mutex);
        return retval;
    }
    pthread_mutex_unlock(&kvaser_mutex);

    //INDIGO_LOG("canSend_driver (Kvaser) :  canWriteWait() send packet, cob_id=%08X, len=%u, flags=%08X, returned value = %d\n",
    //          m->cob_id, m->len, flags, retval);

    printf("SEND: %ld.%ld: ", tv.tv_sec, tv.tv_usec);
    print_message(m);
    printf("\n");
...
canStatus retval = canOK;
    ...
    //fd0 = canOpenChannel(channel, canWANT_EXCLUSIVE | canOPEN_ACCEPT_VIRTUAL |canWANT_EXTENDED);
    //fd0 = canOpenChannel(channel, canWANT_EXCLUSIVE | canOPEN_ACCEPT_VIRTUAL);
    fd0 = canOpenChannel(channel, canOPEN_ACCEPT_VIRTUAL);
    if (fd0 < 0)
    {
